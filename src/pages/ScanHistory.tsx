...
const ScanRow = ({ scan: initialScan }: { scan: Scan }) => {
  const { progress, status } = useScanProgress(initialScan.status === 'scanning' ? initialScan._id : undefined);

  const scan = {
    ...initialScan,
    status: initialScan.status === 'scanning' ? status : initialScan.status,
    progress: initialScan.status === 'scanning' ? progress : initialScan.progress,
  };
